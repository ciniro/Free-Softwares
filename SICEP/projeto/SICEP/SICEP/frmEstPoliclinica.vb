...
Public Class frmEstPoliclinica
    Private Sub Pesquisando(ByVal medicamento As String, ByVal validade As String)
        ...
        ElseIf medicamento.Trim() <> "" And validade.Trim().Equals("") Then
            lblPesquisa.Text = "Pesquisando atualmente por nome de medicamento..."
        ElseIf medicamento.Trim().Equals("") And validade.Trim() <> "" Then
            lblPesquisa.Text = "Pesquisando atualmente por validade de medicamento..."
        Else
            lblPesquisa.Text = "Pesquisando atualmente por nome e validade de medicamento..."
        End If
        Dim est As New EstoquePoliclinica
        If chkMateriais.Checked = True And chkMedicamentos.Checked = True Then
            grdEstoqueSecretaria.DataSource = est.Estoque(medicamento, validade, "")
        ElseIf chkMateriais.Checked = False And chkMedicamentos.Checked = True Then
            grdEstoqueSecretaria.DataSource = est.Estoque(medicamento, validade, "0")
        ElseIf chkMateriais.Checked = True And chkMedicamentos.Checked = False Then
            grdEstoqueSecretaria.DataSource = est.Estoque(medicamento, validade, "1")
        Else
            grdEstoqueSecretaria.DataSource = est.Estoque(medicamento, validade, "")
        End If
    ...
    Private Sub lkImprimir_LinkClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.LinkLabelLinkClickedEventArgs) Handles lkImprimir.LinkClicked
        ...
        frmRelatorios.crvVisualizador.ReportSource = rel.CriarRelEstoquePoliclinica(grdEstoqueSecretaria.DataSource)
        frmRelatorios.Show()
    End Sub
